...
executeProgram :: Types.Statement -> (Types.SingleAssignmentStore, [[Types.EnvironmentMap]], [[Types.SingleAssignmentStore]])
executeProgram x = (sas, listOfEnvLists, listOfSas)
  where (sas, _, stackTuples) = Execution.threadScheduler (Map.empty, Map.empty) [1..] [([(x, Map.empty)], Types.Ready, [], [], "")]
        listOfEnvLists        = map (\(_, _, envList, _, _) -> envList) stackTuples
        listOfSas             = map (\(_, _, _, sasList, _) -> sasList) stackTuples
